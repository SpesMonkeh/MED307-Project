...
namespace P307.Runtime.Hands.MediaPipe
{
	...
	public sealed class PointListAnnotationVoyeur : MonoBehaviour
	{
		...
		void OnHandLandmarkDataCreated(List<HandLandmarkSO> data)
		{
			...
			{
				List<PointAnnotation> points = pointListAnnotation.GetChildren;
		...
		async void BeginPointListAnnotationRetrieval()
		{
			...
			pointCount = pointListAnnotation.GetChildren.Count;
			pointAnnotationsSet?.Invoke(pointListAnnotation.GetChildren);
		}
